...
import { fetchGitHubRepo, cleanupDir } from './gitHubFetcher';
import { runESLint } from './runESLint';
import { runNpmAudit } from './runNpmAudit';
import { runSecretsScanner } from './runSecretsScanner';
import { runA11yAnalyzer } from './runA11yAnalyzer';
import { translateAllPanels } from './claudeTranslator';
...
// Local type definition (consistent with claudeTranslator.ts)
type PanelType = 'code_quality' | 'dependencies' | 'secrets' | 'accessibility';

// Inline Node.js-compatible logger
const logger = {
  debug: (...args: unknown[]) => {
...
// ============================================================================
// Rate Limiting (in-memory, resets at UTC midnight)
// ============================================================================
...
function getResetTimeISO(): string {
...
function checkRateLimit(clientIp: string): RateLimitState {
  const today = getTodayUTC();
  const entry = rateLimitMap.get(clientIp);

  // Reset if new day or no entry
...
function calculateStatus(
  ...
  // If all panels failed/skipped, overall is error
  if (statuses.every((s) => s === 'skipped')) {
    return 'error';
  }

  // If any panel is partial or skipped, overall is partial
...
function buildPanelResult(
  ...
  translatedFindings: TranslatedFinding[]
): PanelResult {
...
export interface OrchestrateScanArgs {
  repoUrl: string;
  scanMode?: 'fast' | 'full';
  clientIp?: string;
}
...
export async function orchestrateScan(
  ...
  try {
    ...
    // ========================================================================
    // Step 4: Build ScanScope from Fetch Metrics
    // ========================================================================
    ...
    const panelsMap = new Map<PanelType, RawFinding[]>([
    ...
    // Unexpected error - should never happen but handle gracefully
    const message = err instanceof Error ? err.message : String(err);
